          ...
          <div class="card-body">
            ...
            <div class="table-responsive">
              <table class="table table-bordered">
                ...
                <tbody>
                  @php($i=1)
                  @foreach ($subdistrict as $cat)
                      
                  <tr>
                    <td>{{$i++}}</td>
                    <td>{{$cat->subdistrict_bn}}</td>
                    <td>{{$cat->subdistrict_en}}</td>
                    <td>{{$cat->district_en}}</td>
                    <td>
                      <a href="{{route('edit.subdistrict', $cat->id)}}" class="btn btn-info">Edit</a>
                      <a href="{{route('delete.subdistrict', $cat->id)}}" onclick="return confirm('Are you sure to delete?')" class="btn btn-danger">Delete</a>
                    </td>
                  </tr>
